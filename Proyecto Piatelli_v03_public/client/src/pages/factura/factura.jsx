...
const guardarReservaEnBD = async () => {
  try {
    ...
    // 3. CREAR UNA SOLA RESERVA para todas las habitaciones
    const reservaData = {
    ...
    console.log("Habitaciones a asociar:", habitacionesReservadas.length);
  
    // 4. GUARDAR TODAS LAS HABITACIONES RESERVADAS en la misma reserva
    if (habitacionesReservadas.length > 0) {
      ...
      console.log("Relación reserva-habitaciones guardada:", habitacionesReservadas.length, "habitaciones");
    }
    ...
    if (actividades && actividades.length > 0) {
      ...
      if (actividadesData && actividadesData.length > 0) {
        const reservaActividadesData = actividadesData.map(actividad => ({
          ...
          fecha_inicio_act: fechas.inicio,
          fecha_fin_act: fechas.fin
        }));
    ...
    // 6. ACTUALIZAR ESTADO DE TODAS LAS HABITACIONES RESERVADAS
    ...
    console.log("Estados de habitaciones actualizados a 'ocupada':", habitacionIds.length, "habitaciones");

    // 7. MOSTRAR CONFIRMACIÓN DETALLADA
    const habitacionesTexto = habitacionesReservadas.map(h => 
      `${h.tipo_habitacion} (Habitación ${h.numero_habitacion})`
    ).join(', ');
    
    const actividadesTexto = actividades.length > 0 ? actividades.join(', ') : 'Ninguna';
    
    console.log(`🎉 ¡Reserva confirmada!\n\n📋 Detalles:\n• Número de reserva: ${reserva.reserva_id}\n• Nombre: ${datosPersona.nombre} ${datosPersona.apellido}\n• Fechas: ${fechas.inicio} a ${fechas.fin}\n• Total: $${calcularTotal()}\n• Habitaciones: ${habitacionesTexto}\n• Actividades: ${actividadesTexto}`);
...
const createPreference = async () => {
  try {
    ...
    // Obtener habitaciones para mostrar en el título
    const habitacionesReservadas = await obtenerHabitacionesParaReserva();
    const habitacionesTexto = habitacionesReservadas.map(h => 
      h.tipo_habitacion
    ).join(', ');
    ...
    const preferenceData = {
      title: "Reserva Hotel - " + habitacionesTexto,
      quantity: 1,
      price: calcularTotal(),
      ...
      metadata: {
        ...
        habitaciones: habitacionesReservadas, // Incluir info detallada de habitaciones
        actividades: actividades,
        total: calcularTotal(),
        diasReserva: calcularDiasReserva(),
      },
    };
